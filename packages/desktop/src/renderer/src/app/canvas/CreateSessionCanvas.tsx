...
import { SkillFileLoadingOverlay, SkillFilesSidebar } from "../skills/SkillFileNavigation"
...
export function GlobalSkillsCanvas({
  ...
  const [skillIdentity, setSkillIdentity] = useState<{
    directoryPath: string | null
    name: string | null
    description: string | null
  }>({ directoryPath: null, name: null, description: null })
  const editorRef = useRef<HTMLTextAreaElement | null>(null)
  const detailID = useId()
  const filesSidebarID = `${detailID}-files-sidebar`
  const currentFileLabelID = `${detailID}-current-file`
  const localFiles = useMemo(() => collectLocalSkillFiles(selectedSkillFiles), [selectedSkillFiles])
  const fileItems = useMemo(() => localFiles.map((file) => ({
  ...
  const selectedLocalFile = localFiles.find((file) => file.path === selectedFilePath) ?? null
  const selectedFileLabel = selectedLocalFile?.relativePath
    ?? selectedFilePath?.split(/[\\/]/).pop()
    ?? t("skillLibrary.local.skillFileName")
  const selectedFileSupportsPreview = Boolean(selectedFilePath?.toLowerCase().endsWith(".md"))
  const effectiveViewMode = selectedFileSupportsPreview ? viewMode : "edit"
  const parsedSkillDocument = useMemo(
    () => isSkillDocumentSelected
      ? parseSkillMarkdownPreview(selectedFileContent)
      : { body: "", metadata: [] as SkillMetadataField[] },
    [isSkillDocumentSelected, selectedFileContent],
  )
  ...
  const cachedIdentity = skillIdentity.directoryPath === selectedSkillDirectoryPath ? skillIdentity : null
  const displayName = metadataName || cachedIdentity?.name || selectedSkillDirectoryName || "Skill"
  const displayDescription = metadataDescription || cachedIdentity?.description
  ...
  useEffect(() => {
    ...
    setViewMode("preview")
    setSkillIdentity({ directoryPath: selectedSkillDirectoryPath, name: null, description: null })
  }, [selectedSkillDirectoryPath])

  useEffect(() => {
    if (!isSkillDocumentSelected) return
    setSkillIdentity((current) => {
      if (
        current.directoryPath === selectedSkillDirectoryPath
        && current.name === metadataName
        && current.description === metadataDescription
      ) return current

      return {
        directoryPath: selectedSkillDirectoryPath,
        name: metadataName,
        description: metadataDescription,
      }
    })
  }, [isSkillDocumentSelected, metadataDescription, metadataName, selectedSkillDirectoryPath])

  useEffect(() => {
    if (!selectedFilePath || isLoadingFile || effectiveViewMode !== "edit") return
    editorRef.current?.focus({ preventScroll: true })
  }, [effectiveViewMode, isLoadingFile, selectedFilePath])

  function handleEditorShellPointerDown(event: PointerEvent<HTMLDivElement>) {
    if (event.target !== event.currentTarget || isLoadingFile || effectiveViewMode !== "edit") return
    editorRef.current?.focus({ preventScroll: true })
  }
  ...
  const editorShellClassName = effectiveViewMode === "preview"
  ...
  return (
    <article className={joinClassNames(
      "global-skills-canvas skill-library-detail-panel skill-library-downloaded-detail skill-library-local-detail",
      ...
      <div className="skill-library-detail-center">
        ...
        <header className="skill-library-detail-header">
          <div className="skill-library-downloaded-identity is-text-only">
            <div className="skill-library-detail-heading">
              ...
              <div className="skill-library-detail-path-row">
                <span className="skill-library-detail-path" title={selectedSkillDirectoryPath}>{selectedSkillDirectoryPath}</span>
              </div>
            </div>
          </div>
        ...
        <div
          className="skill-library-detail-content skill-library-downloaded-tabpanel is-file-view"
          role="region"
          aria-labelledby={currentFileLabelID}
        >
          <div className="skill-library-local-file-editor">
            <div className="global-skills-toolbar">
              ...
              <div className="global-skills-toolbar-actions">
                ...
                >
                  <button
                    className={effectiveViewMode === "edit" ? "global-skills-mode-button is-active" : "global-skills-mode-button"}
                    aria-pressed={effectiveViewMode === "edit"}
                    type="button"
                  ...
                  <button
                    className={effectiveViewMode === "preview" ? "global-skills-mode-button is-active" : "global-skills-mode-button"}
                    aria-pressed={effectiveViewMode === "preview"}
                    disabled={!selectedFileSupportsPreview}
                    type="button"
                ...
                {!selectedFileReadOnly ? (
                  <button className="primary-button" disabled={!isDirty || isSavingFile || isLoadingFile} type="button" onClick={() => void onSave()}>
                    {isSavingFile ? t("app.saving") : t("app.save")}
                  </button>
                ) : null}
              </div>
            </div>
            <div className={editorShellClassName} aria-busy={isLoadingFile} onPointerDown={handleEditorShellPointerDown}>
              {effectiveViewMode === "edit" ? (
                <textarea
                  ...
                  readOnly={selectedFileReadOnly || isLoadingFile}
                  spellCheck={false}
                  value={selectedFileContent}
                  onChange={(event) => {
                    if (selectedFileReadOnly || isLoadingFile) return
                    onChange(event.target.value)
                  }}
                />
              ) : (
                <SkillMarkdownPreview text={selectedFileContent} showMetadata={!isSkillDocumentSelected} />
              )}
