...
- Keep product names, provider names, model names, commands, environment variables, paths, URLs, code, diffs, terminal output, user messages, and existing agent output in their original language; do not translate conversation content in the renderer.
- Pass the selected app locale to each new Agent turn as its default response language. An explicit language request in the current user message takes precedence.
