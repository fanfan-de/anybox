...
export type ExternalApplicationID = "chrome"

export class ExternalApplicationLaunchError extends Error {
  constructor(
    readonly code: "APPLICATION_NOT_FOUND" | "APPLICATION_LAUNCH_FAILED",
    message: string,
    ...
    this.name = "ExternalApplicationLaunchError"
  }
}
...
export interface ExternalApplicationDependencies {
...
export async function resolveExternalApplicationExecutable(
  application: ExternalApplicationID,
  dependencies: ExternalApplicationDependencies = {},
) {
  const accessFile = dependencies.accessFile ?? nodeAccess
  const env = dependencies.env ?? process.env
  const platform = dependencies.platform ?? process.platform
  const homeDir = dependencies.homeDir ?? os.homedir()
  const candidates = application === "chrome"
    ? chromeCandidates(env, platform, homeDir)
    : []
  const pathApi = platformPath(platform)
  ...
  throw new ExternalApplicationLaunchError(
...
function spawnDetached(
  ...
  return new Promise<void>((resolve, reject) => {
    ...
    try {
      ...
      reject(new ExternalApplicationLaunchError(
...
export async function launchExternalApplication(
  ...
  await spawnDetached(
    executablePath,
    dependencies.spawnProcess ?? nodeSpawn,
  )
  return {
    application,
    executablePath,
