...
plugins/Anybox-Plugins/chrome/
  ...
  scripts/                 生成后的 Browser Client 与 Native Host bootstrap
...
`browser-client.mjs`；初始化通过通用 `nodeRepl.requestHost` 宿主桥接并安装
持久的 `agent.browsers`，用于 Backend 发现、标签页查询、页面检查、交互和截图。
Runtime 使用版本化 Browser Contract 获取 capability、机器可读 API Manifest 和动态
文档；客户端预检只提供友好错误，Anybox Agent 是已实现 schema 与 capability 检查的
...
浏览器请求通过与当前 Node REPL tool call 绑定的认证宿主请求直接回到 Anybox Agent。
通用 Node 进程不接收 Browser IPC 凭据，也不暴露 `anybox.browser-runtime`
capability。Rust Native Messaging Host 通过认证本机 IPC 连接 Agent Browser Gateway，
