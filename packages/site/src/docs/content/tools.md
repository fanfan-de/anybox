...
可以把 Agent 想成一个工作人员：Tool 是一件具体器具，Tool Module 是装着同类器具的工具箱，Provider 说明工具箱从哪里来，Permission 则是每次使用前的门锁。

Anybox 没有把所有工具一次性塞给模型。文件、Shell 等核心能力进入常驻目录；Planner 等领域能力按需加载。这样既能减少无关工具占用上下文，也能降低模型选错工具的概率。
...
工具来源与模块目录
  → 判断本轮是否需要显示
  → 应用全局开关、Agent 策略和只读限制
  → 模型选择具体工具
  → 根据这一次的参数、路径和风险判断是否需要审批
  → 执行并把结构化结果交回模型和界面
```

因此，“模块已加载”只表示 Agent 现在可以看到其中的工具；“工具已启用”也不表示所有调用都会自动获批。能力发现和执行授权始终是两件事。
...
| Python 运行时 | 常驻模块；本地工具默认关闭 | 1 | 在当前会话的持久 IPython 环境中运行代码 |
...
这里的“常驻”表示模块属于运行时目录，并不等于工具一定对模型可见。比如 IPython 默认关闭，只有用户明确启用后才会进入候选工具集合。
...
`runtime.python` 为本地工作区提供会话级的持久 Python 环境。

| 工具 | 作用 |
| --- | --- |
| IPython（`ipython`） | 执行 Python 或 IPython 代码；变量、导入和函数会在当前会话的内核存活期间保留。 |

IPython 默认关闭，不支持 SSH 工作区，也不是安全沙箱。代码以当前操作系统用户权限运行，因此每次调用都按高风险执行请求处理，并需要经过调用级权限判断。
