...
export function validateComfyUIBaseURL(value: string | null | undefined) {
  ...
  try {
    ...
    throw new ApiError(400, "COMFYUI_BASE_URL_INVALID", "ComfyUI endpoint must be a valid absolute URL.")
  }
  const hostname = url.hostname.toLowerCase()
  if (url.protocol !== "http:" && url.protocol !== "https:") {
    throw new ApiError(400, "COMFYUI_BASE_URL_INVALID", "ComfyUI endpoint must use HTTP or HTTPS.")
  }
  if (!["localhost", "127.0.0.1", "[::1]", "::1"].includes(hostname)) {
    throw new ApiError(
      400,
      "COMFYUI_BASE_URL_NOT_LOCAL",
      "ComfyUI endpoint must use localhost, 127.0.0.1, or ::1.",
    )
  }
  if (url.username || url.password || url.search || url.hash) {
    throw new ApiError(
      ...
      "ComfyUI endpoint cannot contain credentials, query parameters, or a fragment.",
    )
  }
  if (url.port === "0") {
    throw new ApiError(400, "COMFYUI_BASE_URL_INVALID", "ComfyUI endpoint must use a valid TCP port.")
  }
  if (url.pathname !== "/" && url.pathname !== "") {
    throw new ApiError(400, "COMFYUI_BASE_URL_INVALID", "ComfyUI endpoint cannot contain a path.")
  }
  return url.origin
}
...
export async function assertComfyUIEndpointResolvesToLoopback(baseURL: string) {
  const url = new URL(validateComfyUIBaseURL(baseURL))
  const hostname = url.hostname.toLowerCase()
  ...
  if (
    ...
    throw new ApiError(
      400,
      "COMFYUI_BASE_URL_NOT_LOCAL",
      "ComfyUI endpoint resolved outside the local loopback interface.",
    )
  }
}
...
export async function fetchComfyUI(
  ...
  try {
    return await fetch(urlForComfyUI(baseURL, route), {
      ...init,
