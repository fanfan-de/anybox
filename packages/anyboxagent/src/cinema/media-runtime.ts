...
async function existingExecutable(explicit: string | undefined, names: string[]) {
  if (explicit?.trim()) {
    const resolved = path.resolve(explicit.trim())
    if (!fs.existsSync(resolved)) throw new Error(`Configured media binary does not exist: ${resolved}`)
    return resolved
  }
