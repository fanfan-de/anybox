...
Deliver Beta is a separate non-publishing path for Windows x64, macOS arm64, and Linux x64. `.github/workflows/cinema-deliver-beta.yml` builds the pinned FFmpeg revision on each native runner, exercises the target H.264/AAC encoder, bundles that exact FFmpeg/FFprobe pair plus license, notices, configure output, source metadata, and build recipe, and then creates unsigned `Anybox-Deliver-Beta` installers. Linux additionally binds the exact x264 and zlib sources and license files. The packaged Agent remains strict about using its bundled pair, the manifest binds the binary hashes into a Beta runtime ID, and the UI labels the capability **Deliver Beta**. The workflow never marks the runtime release-approved and never publishes artifacts to an update channel.
...
Use `pnpm --filter anybox-desktop-agent run dist:deliver-preview --dir --win` (or `--mac` / `--linux`) for an unpacked CI/local rehearsal. Omitting `--dir` can create a clearly named, non-publishing preview installer when a human installed-path exercise is needed. The wrapper enables `VITE_CINEMA_DELIVER_DEV=1`, runs the non-strict technical verifier, places electron-builder artifacts under `packages/desktop/dist/deliver-preview`, forces `--publish never`, and rejects other publish policies. It never replaces release-strict `dist`, `dist:dir`, or `dist:publish`, and its output is not production approval evidence.

For a usable installer with a bundled native runtime, dispatch `.github/workflows/cinema-deliver-beta.yml`. The Windows, Apple Silicon, and Linux jobs install the pinned build dependencies, build and smoke their own FFmpeg runtime, and then invoke `dist:deliver-beta`; a missing binary, license/source/build material, reviewed subtitle font, or OFL license fails the package. Linux also fails closed if libx264, zlib/PNG, or their corresponding source/license material is absent. Local platform maintainers can run the same command after setting `ANYBOX_FFMPEG_BINARY`, `ANYBOX_FFPROBE_BINARY`, and `ANYBOX_MEDIA_RUNTIME_MATERIALS_DIR` to the stage produced by `build-media-runtime.sh`. Beta artifacts remain manual downloads and are not eligible for `dist:publish`.
...
10. Run `.github/workflows/cinema-deliver-release-candidates.yml` from the protected `master` branch after all three runtime targets are promoted and approved. The jobs use the protected `cinema-deliver-signing` environment and do not run until the Cinema unit, Edit E2E, Deliver E2E, Desktop gate, and Agent Cinema regressions pass. The workflow performs release-strict packaging and produces `cinema-deliver-win32-x64`, `cinema-deliver-darwin-arm64`, and `cinema-deliver-linux-x64`; each artifact contains only the required installer/update files and a SHA-256 manifest. The Linux set must include an AppImage with its updater blockmap embedded, a Debian package, and `latest-linux.yml`. Install those exact primary installers, complete three redacted restart records, and complete a copy of [cinema-deliver-release-approval.template.json](./cinema-deliver-release-approval.template.json) with separate license, product, and security approval, including the Linux GPL/corresponding-source scopes. Dispatch `.github/workflows/cinema-deliver-release.yml` with the candidate run ID and repository evidence paths, first with `publish: false`. Then publish an `internal-rc` using that rehearsal run ID. Public publishing requires both the rehearsal run ID and successful internal-RC run ID; their candidate SHA and all three complete artifact manifests must be byte-identical. The release workflow re-hashes every publishable file, validates updater metadata, binds encoder/version/runtime facts, verifies Windows and Apple signatures, validates Linux package structure, and stages one synchronized GitHub/COS release before publishing it.
