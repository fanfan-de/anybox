...
> Status: D5 working document, 2026-07-11. Deliver opens as Beta by default (`VITE_CINEMA_DELIVER_BETA=0` is the emergency UI kill switch). Windows x64 and Apple Silicon macOS Beta installers bundle a native, smoke-tested FFmpeg runtime. Formal release approval remains separate from Beta availability.
...
For a usable installer with a bundled native runtime, dispatch `.github/workflows/cinema-deliver-beta.yml`. The Windows and Apple Silicon jobs install the same pinned subtitle build dependencies, build and smoke their own FFmpeg runtime, and then invoke `dist:deliver-beta`; a missing binary, license/source/build material, reviewed subtitle font, or OFL license fails the package. Local platform maintainers can run the same command after setting `ANYBOX_FFMPEG_BINARY`, `ANYBOX_FFPROBE_BINARY`, and `ANYBOX_MEDIA_RUNTIME_MATERIALS_DIR` to the stage produced by `build-media-runtime.sh`. Beta artifacts remain manual downloads and are not eligible for `dist:publish`.
...
10. Run `.github/workflows/cinema-deliver-release-candidates.yml` from the protected `master` branch after both runtime targets are promoted and approved. The signing jobs use the protected `cinema-deliver-signing` environment and do not run until the Cinema unit, current Edit E2E suite, fourteen Deliver E2E, Desktop gate, and Agent Cinema regressions pass. The workflow performs release-strict packaging and produces `cinema-deliver-win32-x64` plus `cinema-deliver-darwin-arm64`; each artifact contains only the required installer/update files and a SHA-256 manifest. After final DMG notarization stapling changes the container bytes, the workflow rebuilds the DMG blockmap and refreshes its `latest-mac.yml` SHA-512/size before locking the candidate manifest. Install those exact primary installers, complete the two redacted restart records, and complete a copy of [cinema-deliver-release-approval.template.json](./cinema-deliver-release-approval.template.json) with separate license, product, and security approval. Commit the redacted records without changing the candidate identity they name. Dispatch `.github/workflows/cinema-deliver-release.yml` with the candidate run ID and repository evidence paths, first with `publish: false`. Then publish an `internal-rc` using that rehearsal run ID. Public publishing requires both the rehearsal run ID and successful internal-RC run ID; their candidate SHA and both complete artifact manifests must be byte-identical. The release workflow re-hashes every publishable file, validates updater version/path/SHA-512/size, binds encoder/version/runtime facts, verifies the expected Anybox Authenticode subject/thumbprint and Apple Team ID in both the DMG and updater ZIP, and validates Gatekeeper plus DMG notarization/stapling. Public release first stages a GitHub draft, then switches the synchronized COS download manifest/Windows generic update feed for both platforms, and finally publishes the existing GitHub draft.
