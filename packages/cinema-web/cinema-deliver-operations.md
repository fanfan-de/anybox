...
For a usable installer with a bundled native runtime, dispatch `.github/workflows/cinema-deliver-beta.yml`. The Windows and Apple Silicon jobs build and smoke their own FFmpeg runtime before invoking `dist:deliver-beta`; a missing binary or any of the five required license/source/build material files fails the package. Local platform maintainers can run the same command after setting `ANYBOX_FFMPEG_BINARY`, `ANYBOX_FFPROBE_BINARY`, and `ANYBOX_MEDIA_RUNTIME_MATERIALS_DIR` to the stage produced by `build-media-runtime.sh`. Beta artifacts remain manual downloads and are not eligible for `dist:publish`.
