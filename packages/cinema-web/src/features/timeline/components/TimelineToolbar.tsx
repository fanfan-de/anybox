import { Magnet, Maximize2, Redo2, Scissors, Undo2, ZoomIn, ZoomOut } from "lucide-react"
...
export function TimelineToolbar({
  ...
  return (
    <div className="cinema-timeline-toolbar" role="toolbar" aria-label="Timeline tools">
      {tools.map(([label, Icon, onClick, disabled]) => (
        <button key={label} type="button" className={label === "Snap" && snapEnabled ? "is-active" : ""} aria-label={label} title={label} aria-pressed={label === "Snap" ? snapEnabled : undefined} disabled={disabled} onClick={onClick}>
