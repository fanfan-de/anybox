...
ffmpeg_revision="${CINEMA_TOOLCHAIN_FFMPEG_REVISION:-8ad6288553}"
x264_revision="${CINEMA_TOOLCHAIN_X264_REVISION:-b35605ace3ddf7c1a5d67a2eb553f034aef41d55}"
zlib_version="1.3.1"
zlib_sha256="9a93b2b7dfdac77ceba5a558a580e74667dd6fede4585b91eefb60f03b72df23"
node_binary="${CINEMA_TOOLCHAIN_NODE_BINARY:-node}"
script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
plugin_dir="$(cd "${script_dir}/../.." && pwd)"
work_dir="${CINEMA_TOOLCHAIN_WORK_DIR:-${plugin_dir}/build/media-runtime-source-${platform}-${arch}}"
output_dir="${CINEMA_TOOLCHAIN_OUTPUT_DIR:-${plugin_dir}/build/media-runtime-candidates/${platform}-${arch}}"
...
if [[ "${CINEMA_TOOLCHAIN_RUN_SMOKE:-0}" != "1" ]]; then
  echo "media runtime candidates require CINEMA_TOOLCHAIN_RUN_SMOKE=1 so render and subtitle evidence is archive-bound" >&2
  exit 2
fi
...
build_linux_dependencies() {
  ...
  local jobs="${CINEMA_TOOLCHAIN_JOBS:-2}"
...
make -j"${CINEMA_TOOLCHAIN_JOBS:-2}" "${ffmpeg_name}" "${ffprobe_name}"
popd >/dev/null
...
Build recipe: plugins/Anybox-Plugins/cinema/toolchain/scripts/build-media-runtime.sh
...
Release authorization is recorded outside this archive in plugins/Anybox-Plugins/cinema/toolchain.lock.json.
