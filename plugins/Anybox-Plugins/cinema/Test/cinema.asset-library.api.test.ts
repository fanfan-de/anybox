...
import { CinemaAssetCatalogSchema } from "@anybox/cinema-plugin/contracts"
...
beforeEach(() => {
  ...
  if (ffmpeg && ffprobe) restores.push(setMediaToolPathsForTest({ ffmpeg, ffprobe }))
})
...
async function createCinemaProject() {
  ...
  return { app, root, projectID: project.id }
}
...
describe("cinema asset library api", () => {
  ...
  test("keeps crop derivatives in their source folder and processes media in the background", async () => {
    ...
    expect((await app.request(projectLibraryURL(projectID, "/state"))).status).toBe(200)
