...
} from "../src/domain/render-runtime"
import { resolveMediaToolPaths } from "../src/domain/media-runtime"
import { CinemaRoutes } from "../src/api/cinema-routes"
...
describe("cinema render runtime", () => {
 ...
 A..... aac                  AAC (Advanced Audio Coding)
  ...
  test("selects an auditable path-free runtime binding", async () => {
    ...
    expect(resolveCinemaRenderRuntimeID(undefined, "linux", "7.1.1")).toBe("dev-linux-7.1.1")
  ...
  test("plugin toolchain discovery never falls back to PATH or legacy environment variables", async () => {
    await expect(resolveMediaToolPaths({ ANYBOX_MEDIA_RUNTIME_STRICT: "1" }))
      .rejects.toThrow("Install the reviewed Cinema media toolchain")
