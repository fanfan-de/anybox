...
import { initializeCinemaProject, resetProjectsForTest } from "#project/project.ts"
import {
  initializeCinemaAssetLibrary,
  setCinemaAssetLibraryCatalogWriteFailureForTest,
} from "#cinema/asset-library.ts"
import { setMediaToolPathsForTest } from "#cinema/media-runtime.ts"
...
const cinemaWebDist = path.resolve(moduleDirectory, "../../web")
...
const runtimeDataRoot = path.join(projectRoot, ".runtime-data")

Global.configureRuntimePaths({
  data: runtimeDataRoot,
  cache: path.join(runtimeDataRoot, "cache"),
  log: path.join(runtimeDataRoot, "logs"),
})
resetProjectsForTest()

const runtime = { app: createServerApp({ mode: "test" }) }

const project = await initializeCinemaProject(projectRoot)
...
const fixtureMp4 = Buffer.from("", "base64")
...
function fixtureWav() {
  ...
  for (let index = 0; index < sampleCount; index += 1) {
    bytes.writeInt16LE(Math.round(Math.sin(2 * Math.PI * 660 * index / sampleRate) * 6_000), 44 + index * 2)
  }
  return bytes
}

const e2eFFmpeg = process.env.CINEMA_E2E_FFMPEG ?? Bun.which("ffmpeg")
const e2eFFprobe = process.env.CINEMA_E2E_FFPROBE ?? Bun.which("ffprobe")
if (e2eFFmpeg && e2eFFprobe) {
  setMediaToolPathsForTest({ ffmpeg: e2eFFmpeg, ffprobe: e2eFFprobe, runtimeID: "cinema-e2e-system-tools" })
}
...
async function ensureFixtureMedia() {
  ...
  try {
    ...
    await writeFile(videoPath, fixtureMp4)
  }
  try {
    await stat(audioPath)
  } catch {
    await writeFile(audioPath, fixtureWav())
  }
...
runtime.app.get("/healthz", (context) => context.json({ ready: true, mode: "test" }))
...
async function serveCinemaWeb(request: Request) {
  if (request.method !== "GET") return undefined
  const url = new URL(request.url)
  if (url.pathname.startsWith("/api/") || url.pathname.startsWith("/e2e/") || url.pathname === "/healthz") return undefined
  const requestPath = decodeURIComponent(url.pathname.replace(/^\/+/, "").replace(/^cinema\/?/, ""))
  if (requestPath.split(/[\\/]/).some((segment) => segment === "..")) return undefined
  const webRoot = path.resolve(cinemaWebDist)
  let candidate = path.resolve(webRoot, requestPath || "index.html")
  if (!candidate.startsWith(`${webRoot}${path.sep}`) && candidate !== path.join(webRoot, "index.html")) return undefined
...
const server = Bun.serve({
  ...
  async fetch(request) {
    ...
    return await serveCinemaWeb(request) ?? response
  },
})
