...
`selection`、`playhead`、`viewport` 和 render job 状态都是 UI 或 Deliver 域状态，不写入 Timeline 文档。持久化事实来源为 `@anybox/cinema-plugin/contracts/timeline`。
...
完整严格契约以 `@anybox/cinema-plugin/contracts/render` 为准。Job/API 不返回绝对输入路径、输出路径或完整 FFmpeg 命令。
