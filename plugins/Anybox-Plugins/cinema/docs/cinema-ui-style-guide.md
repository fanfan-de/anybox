...
- 非编辑态使用受限 Markdown 渲染，支持标题、段落、强调、列表、引用、分隔线和单换行；编辑态 textarea 始终展示并保存原始 Markdown 源码，不新增独立文档格式或 Canvas schema 字段。
- Markdown 预览必须跳过原始 HTML，不加载远程图片，不创建可点击链接，不启用宽表格；图片语法只显示替代文本，代码块必须在节点宽度内换行。
...
- 素材库顶栏只保留上传、刷新和关闭。位于具体文件夹时直接上传到当前目录；位于根目录或搜索结果时先选择目标文件夹；拖到文件夹行时直接上传到该文件夹。
- 新建文件夹只放在空白区和文件夹的右键菜单中，不提供显眼的顶栏入口。菜单必须使用 portal/fixed 定位，支持视口避让、键盘导航、`Escape`、外部点击与焦点恢复。
- 删除在产品层表现为直接删除：单个素材不确认，文件夹或多选只确认一次；完成后提供 10 秒撤销。仍被 Canvas、Timeline 或任务引用的素材必须整批阻止删除，不能产生部分结果。
- 不展示应用内回收站、角标、恢复列表或清空入口。内部 `trashed` 只表示 10 秒撤销期与崩溃回滚隔离状态，跨工作台统一显示为“已删除”，不得引导用户打开回收站。
- Canvas 新素材节点只持久化服务端返回的 canonical `assetRef`，不得由前端拼接或保存托管文件的绝对路径。
- 项目和个人素材都以引用方式加入 Canvas。个人引用必须显示“个人素材”和可移植性提示；missing、processing 与内部 `trashed` 降级状态必须有非颜色文案，其中 `trashed` 对用户表述为“已删除”。
